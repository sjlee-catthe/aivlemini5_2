...
export default {
  ...
  mounted() {
    this.repository = new BookRepository(axios, 'books');
    this.search(null);
  },
  methods: {
    async search(query) {
    if (
      ...
      !query.parameters ||           // parameters가 없거나
      Object.keys(query.parameters).length === 0 // parameters가 빈 객체
    ) {
      // 검색조건이 없는 경우: 전체 조회
      const books = await this.repository.find(null);
      this.value = Array.isArray(books) ? books : [];
    } else {
      // 검색조건이 있을 때
      const books = await this.repository.find(query);
      this.value = Array.isArray(books) ? books : [];
    }
    return this.value;
  },
